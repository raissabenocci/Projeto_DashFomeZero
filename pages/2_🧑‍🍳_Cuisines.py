...
#
# 
def findCuisine1( df_def, cols, strMid, strMetric, colorBkg ):
    '''  FUNÇÂO  para achar cuisines baseado em qual coluna
    '''
    df_aux =  df_def.loc[ : , cols] .groupby( [ cols[0] ] ).agg( {cols[-1]: strMetric} )
...
with st.container():
    ...
    with col2:    # - - - - - - - - - - - - - - - - - - - - -   Culinária(s) com mais restaurantes
        ...
        strMetric = 'count'
        findCuisine1( df_def, cols, strMid, strMetric, colorBkg='#b3ffe6')
        #
    with col3:    # - - - - - - - - - - - - - - - - - - - - -   Culinária(s) em mais países
        ...
        strMetric = 'count'
        findCuisine1( df_def, cols, strMid, strMetric, colorBkg='#00e6e6')
        #
    with col4:    # - - - - - - - - - - - - - - - - - - - - -   Culinária(s) com melhor avaliação
        ...
        strMetric = 'mean'
        findCuisine1( df_def, cols, stringMid, strMetric, colorBkg='#b3ffe6')
        #
...
with st.container():
    ...
    # with st.container():
    #     if topBottom == 'Top':
    #         markdown_content = 'Mostrar o dataframe das Top Culinárias?'
    #     elif topBottom == 'Bottom':
    #         markdown_content = 'Mostrar o dataframe das Bottom Culinárias?'
    #     on = st.toggle( markdown_content , value=False )
    #     if on:
    #         markdown_content = f'<h4 style="text-align: center;">  Detalhes destas Culinárias <br> </h4>'
    #         st.markdown(  markdown_content,  unsafe_allow_html=True  )
    #         st.dataframe( df_auxTopBottom , use_container_width=True )
    # #
#
st.markdown( """---""" )
#
